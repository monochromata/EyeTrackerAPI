...
public abstract class Calibrator {
	...
	// protected boolean toggleOn = true;
	 ...
	 * 
	 * @param iViewIP
	 *            - the ip of the eye tracker machine as a string (example
	 *            \"129.21.175.1\")
	 * @param Port
	 *            - the port the eye tracker machine is usign (example 4455)
	 * @param localPort
	 *            - the local port that eye tracker will send packets to
	 * @param screenWidth
	 *            - width of the stimulus screen as an int
	 * @param screenHeight
	 *            - height of the stimulus screen as an int
	 */
	public Calibrator(String eyeTrackerIP, int eyeTrackerPort, int localPort,
			int screenWidth, int screenHeight, int stimulusDisplayIndex) {
		...
		// create an array of bytes that represents the ip address of the eye
		// tracker system
		...
		// try to get the address from the byte array, print out stack trace and
		// a message saying
		...
		try {
			...
			e.printStackTrace();
			System.err
					.println("could not find eye tracker system at specified IP: "
							+ eyeTrackerIP);
		}
	 ...
	 * Calibration is then started. The first point waits for manual accept
	 * (from space bar in our case). All following points are auto accepted. eye
	 ...
	 * accuracy.
	 * 
	 * @return - the resulting string array produced by the validation done
	 *         after calibration (see validation method for more details)
	 */
	public abstract String[][] calibrate();

	/**
	 * Performs validation mimicking the protocol SMI Experiment center uses.
	 * Verifies the point at 1/4 the screen width and height, then 3/4 the
	 * screen width and height, then 3/4 width 1/4 height, and finally 1/4 width
	 * and 3/4 height. The results sent back from eye tracker are stored in an
	 * array and returned.
	 * 
	 * @return the resulting string array produced by validation, eye tracker
	 *         sends back strings containing the average x,y coordinate of the
	 *         users gaze during validation, and the std dev in the x and y
	 *         directions in visual degrees.
	 */
	...
	protected double[] calculateDeviations(String[][] validationStrings) {
		...
		// loop through the results and add the x and y values to the
		// corresponding running sum
		for (String str : validationStrings[1]) {
			String[] tokens = str.split(" ");

			xRunningSum += new Double(tokens[3].substring(0,
					tokens[3].length() - 2));
			yRunningSum += new Double(tokens[4].substring(0,
					tokens[4].length() - 2));

		}

		// calculate the mean deviation by dividing by the length of the results
		// array
	...
	/*
	 * Advances the calibration circle passed in as a parameter to the next
	 * calibration point in the array of calibration points received from eye
	 * tracker.
	 * 
	 * @param calibrationCircle - the CalibrationCircle object to advance
	 */
	private void advanceToNextCalibrationPoint(
		...
		if (calibrationPointsIndex < calibrationPoints.length) {
			...
			advanceToNextCalibrationPoint(calibrationCircle,
					calibrationPoints[calibrationPointsIndex - 1],
					calibrationPoints[calibrationPointsIndex]);

		}

	}

	protected abstract void advanceToNextCalibrationPoint(
			CalibrationCircle calibrationCircle, Point oldPoint, Point newPoint);

	/*
	 * Updates the calibration area. If the calibration area is null, creates a
	 * new one.
	 */
	protected abstract void updateCalibrationArea();

	/*
	 * This method takes in the results array from a validation and displays
	 * grey markers where the validation points were and red markers where eye
	 * tracker measure the users gaze. This is done using the
	 * ValidationVisualization inner class.
	 */
	protected void showResults(String[][] results) {
		...
		for (String resultString : results[1]) {
			...
			validationVisualizer.addUserPoint(
					new Integer(resultTokens[1].substring(0,
							resultTokens[1].indexOf("."))),
					new Integer(resultTokens[2].substring(0,
							resultTokens[2].indexOf("."))));
			i++;
		}
		...
		for (String pointUsed : results[0]) {
			...
			validationVisualizer.addCheckPoint(new Integer(tokens[0]),
	 ...
	 */
	protected void setCalibrationCircleLocation(
			CalibrationCircle calibrationCircle, int x, int y) {
	 ...
	 * connection with the eye tracker has been established, use
	 * tesConnection().
	 * 
	...
	/*
	 * A runnable for forcing the calibration circle to update itself. Needed
	 * when the calibration runs after a retry command is received from the
	 * user, because the circle is in a strange location
	 */
	 ...
	 * 
	 * @param stimulusWidth
	 *            - the width of the stimulus display
	 * @param stimulusHeight
	 *            - the height of the stimulus display
	 */
	 ...
	 * 
	 * @param iViewIP
	 *            - the IP address of the eye tracker machine
	 * @param iViewPort
	 *            - the port the eye tracker machine is using
	 */
	 ...
	 * 
	 * @param localPort
	 *            - the port eye tracker is sending packets to
	 */
	...
	/*
	 * A class that creates a graphical representation of the results of the
	 * validation. Displays black marks where the validation points were and red
	 * marks where the eye tracker system measured the users gaze.
	 */
	class ValidationVisualization extends JComponent {
		...
		public void paint(Graphics g) {
			...
			for (Point pnt : greyCheckPoints) {
				g2d.drawLine(pnt.x - (LINE_LENGTH / 2), pnt.y, pnt.x
						+ (LINE_LENGTH / 2), pnt.y);
				g2d.drawLine(pnt.x, pnt.y - (LINE_LENGTH / 2), pnt.x, pnt.y
						+ (LINE_LENGTH / 2));
			}
		...
		/*
		 * Adds a new validation point. This is where the validation was
		 * performed. Will be displayed as a black mark.
		 * 
		 * @param x - the x coordinate
		 * 
		 * @param y - the y coordinate
		...
		/*
		 * Adds a new user point. This is where the users gaze was measured.
		 * Will be displayed as a red mark.
		 * 
		 * @param x - the x coordinate
		 * 
		 * @param y - the y coordinate
