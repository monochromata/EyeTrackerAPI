...
public abstract class EyeTrackingApp {
	 ...
	 * implementations will need to be used. The implementation used is
	 * dependent on the eye tracker being used. Furthermore, if no
	 * implementation exists for the type of eye tracker being used, this class
	 * can be extended to make a new client program for the eye tracker being
	 * used.
	 */
	 ...
	 * the tracker. This is an abstract class, so a custom implementation of
	 * this will need to be created based on what is to be done with the data
	 * coming from the tracker. For example, if you wish to display a cursor
	 * where the user is looking, the custom EyeTrackingMediator should draw a
	 * cursor at the coordinates of the users gaze everytime it refreshes.
	 */
