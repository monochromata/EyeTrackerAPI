...
/**
 * This class represents a calibration circle. The circle will be white, with a
 * red center. This circle can then be moved to different places on the screen
 * to perform a calibration using eye tracker.
 */
public class CalibrationCircle extends JComponent {
	...
	public void paint(Graphics g) {
		...
		g2d.fillOval(x - (OUTER_CIRCLE_DIAMETER / 2), y
				- (OUTER_CIRCLE_DIAMETER / 2), OUTER_CIRCLE_DIAMETER,
				OUTER_CIRCLE_DIAMETER);

		// paint inner red circle
		g2d.setPaint(new Color(250, 0, 0));
		g2d.fillOval(x - (INNER_CIRCLE_DIAMETER / 2), y
				- (INNER_CIRCLE_DIAMETER / 2), INNER_CIRCLE_DIAMETER,
